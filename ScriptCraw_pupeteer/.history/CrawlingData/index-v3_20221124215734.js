...
const pro = async () => {
  ...
  const giai = await page.evaluate(() => {
    return Array.from(document.querySelectorAll("#TN_prize8_item0")).map(
      (e) => e.textContent
    );
    // document.querySelectorAll(".main .main-content .content-left .section .section-content .table-xsmn tbody tr");
  });
  const list = [];
  var ketQua;
  var arr = [];
