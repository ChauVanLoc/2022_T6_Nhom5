...
const pro = async (link) => {
  let day = link.substring(25, 35);
  let id = day.replaceAll("-", "");
  const idDai = `#mn_kqngay_${idDai}_kq > table.table-result.table-xsmn > thead > tr > th > h3 > a`;
  const idGiai = `#mn_kqngay_${idGiai}_kq > table.table-result.table-xsmn > tbody > tr`;
