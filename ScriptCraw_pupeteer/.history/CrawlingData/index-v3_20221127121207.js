...
const pro = async (link) => {
  const day = link.substring(25, 35);
  const id = day.replaceAll("-", "");
  ...
  const dai = await page.evaluate(() => {
    return [
      ...document.querySelectorAll(
        `#mn_kqngay_${id}_kq > table.table-result.table-xsmn > thead > tr > th > h3 > a`
      ),
    ].map((e) => e.textContent);
  });
  const giai = await page.$$(
    `#mn_kqngay_${id}_kq > table.table-result.table-xsmn > tbody > tr`
  );
  var tmp,
    str = "";
  var arr = [];
