  ...
  const dai = await page.evaluate(() => {
    return [
      ...document.querySelectorAll(
        "#mn_kqngay_08112022_kq > table.table-result.table-xsmn > thead > tr > th > h3 > a"
      ),
    ].map((e) => e.textContent);
  });
  const giai = await page.$$(
    "#mn_kqngay_08112022_kq > table.table-result.table-xsmn > tbody > tr"
  );
  var tmp,
    str = "";
  var arr = [];
...
pro(`https://xoso.com.vn/xsmn-08-11-2022.html`).then((e) =>
  writeDataIntoCSV(`DataXS_ChauVanLoc.${time}-08-11-2022.csv`, e)
);
