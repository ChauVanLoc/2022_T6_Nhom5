...
const pro = async () => {
  ...
  const giai = await page.evaluate(() => {
    return [
      ...document.querySelectorAll(
        "#mn_kqngay_24112022_kq > table.table-result.table-xsmn > tbody > tr > td"
      ),
    ][0].querySelector("td span").textContent;
  });
  console.log("Length dai: ", dai.length);
  console.log("Length giai: ", giai.length);
  console.log(giai[0].querySelector("td span").innerHTML);
  var tmp,
    arr,
    resultArr = [];
  for (let x = 0; x > dai.length; x++) {
    console.log("Dai chay");
    for (let y = 0; y > giai.length; y++) {
      console.log("Giai chay");
      tmp = giai[y].querySelectorAll("td");
      console.log("tmp " + tmp);
      Array.isArray([...tmp[x].querySelectorAll("span")])
        ? arr.push(
            [...tmp[x].querySelectorAll("span")]
              .map((e) => e.textContent)
              .join(", ")
          )
        : arr.push(tmp[x].querySelector("span").textContent);
    }
    // console.log(arr);
    resultArr.push(arr);
    arr.length = 0;
